...
class GetPortfoliosByAccountIdOperation(private val request: MobileRequest, private val session: WebSocketSession,
                                                                        private val portfolioRepository: IPortfolioRepository, private val accountId: String) : Runnable {

    override fun run() {
        synchronized(session) {
            try {
                val portfolioContent = portfolioRepository.getPortfolios(accountId)
